loadstring(game:HttpGet("https://nb0.xyz/scripts/adoptme_bypass_o.lua"))()
wait(20)
...
GoToMainMap = function()
    ...
    repeat task.wait(1) until game.workspace.Interiors:FindFirstChildWhichIsA("Model") and game.workspace.Interiors:FindFirstChild("Winter2024Shop")
    if workspace:FindFirstChildWhichIsA("Terrain") then workspace.Terrain:Clear() end
    return false
end
...
function GetLobby()
    ...
    if (workspace.Interiors["Winter2024Shop"].JoinZones.SpleefMinigame.Ring.Position - HRP.Position).Magnitude <= 15 then
        return true
    end
    return false
end
...
while task.wait(1) do
    if GetMainMap() then
        pcall(function()
            if GetLobby() then
                if (ClientData.get_data()[game.Players.LocalPlayer.Name]["spleef_minigame_cycle_timestamp"]["timestamp"] - os.time() > 150) and (not GameLoading()) and (not farmedG) then
                ...
                pcall(function()
                    HRP.Anchored = true
                    HRP.CFrame = CFrame.new(-15956, 11151, -15888) * CFrame.Angles(0, 0, 0)
                    Player.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Landed)
                    CreateTempPart()
                    HRP.Anchored = true
                    task.wait(1)
